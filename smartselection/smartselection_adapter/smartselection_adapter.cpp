...
int SmartSelectionAdapter::Init() {
    ...
    if (snap_ret != 0 || !snapmapper_) {
        DLOGW("Init(): failed to initialize ISnapMapper (ret=%d); DoEnqueue() will fail",
              snap_ret);
    }
...
int SmartSelectionAdapter::ProcessOps(SmartSelectionOp ops, const sdm::GenericPayload& input,
    ...
    switch (ops) {
        case kSSInit:          return DoInit(input, output);
        case kSSReconfigure:   return DoReconfigure(input, output);
        case kSSEnqueue:       return DoEnqueue(input, output);
        case kSSFlushByConfig: return DoFlushByConfig(input, output);
        case kSSFlushAll:      return DoFlushAll(input, output);
        case kSSDeleteByConfig:return DoDeleteByConfig(input, output);
        case kSSDeleteAll:     return DoDeleteAll(input, output);
        case kSSDeinit:        return DoDeinit(input, output);
        case kSSWaitUntilIdle: return DoWaitUntilIdle(input, output);
        default:
...
void SmartSelectionAdapter::OnEmitResult(const QaiorSS_EmitResult_t* result) {
    ...
    DLOGD("OnEmitResult: selected=%d rejected=%d",
    ...
    auto lookup_ahb = [this](AHardwareBuffer* ahb,
        ...
        if (!ahb) return {false, nullptr};
        std::lock_guard<std::mutex> lock(pending_ahb_mutex_);
        auto it = pending_ahbs_.find(ahb);
        if (it == pending_ahbs_.end()) {
            DLOGW("OnEmitResult: AHB %p (%s) not in pending set — already released, skipping",
                  ahb, tag);
        ...
        DLOGD("OnEmitResult: AHB %p (%s) removed from pending set, native_handle=%p",
              ahb, tag, nh);
        return {true, nh};
    };
    ...
    auto convert_frame = [&](const QaiorSS_FrameHandle_t& src, int idx,
                              const char* tag, SmartSelectionFrame& dst) -> bool {
        ...
        switch (src.type) {
            case QAIOR_SS_FRAME_PARCELFD:
                ...
                if (src.parcelFd >= 0) {
                    ...
                    if (dst.parcel_fd < 0) {
                        DLOGW("OnEmitResult: dup() failed for %s frame %d: %s",
                              tag, idx, strerror(errno));
                        return false;
                    }
                }
                break;
            case QAIOR_SS_FRAME_AHARDWAREBUFFER: {
                ...
                if (!found) return false;  // concurrent Deinit released it; skip frame
    ...
    for (int32_t i = 0; i < result->selectedCount; ++i) {
        ...
        if (!convert_frame(result->selectedFrames[i], i, "selected", fr)) continue;
    ...
    for (int32_t i = 0; i < result->rejectedCount; ++i) {
        SmartSelectionFrame fr;
        if (!convert_frame(result->rejectedFrames[i], i, "rejected", fr)) continue;
...
int SmartSelectionAdapter::DoInit(const sdm::GenericPayload& input, sdm::GenericPayload* output) {
    ...
    QaiorSS_Context_t* new_context = init_fn(new_pipeline, config_to_use.c_str(),
                                              EmitCallbackTrampoline, this);
    if (!new_context) {
    ...
    // Store results under lock; check for concurrent Deinit() or concurrent DoInit().
    {
        ...
        if (!init_done_) {
            ...
            if (deinit_fn) deinit_fn(new_pipeline, new_context);
            destroy_fn(new_pipeline);
            return -EINVAL;
        }
        if (pipeline_) {
            ...
            DLOGE("DoInit: pipeline already set by concurrent DoInit(); "
...
int SmartSelectionAdapter::DoReconfigure(const sdm::GenericPayload& input,
    ...
    {
        std::lock_guard<std::mutex> lock(mutex_);
        if (!pipeline_ || !context_) { DLOGE("Pipeline not initialized"); return -EINVAL; }
...
int SmartSelectionAdapter::DoEnqueue(const sdm::GenericPayload& input,
    ...
    // Single enqueue call (no mutex_ held).
    // Pass 'this' as per-frame cookie: the library stores it by uid and passes it back
    // to the emit callback (not the QaiorSS_init user_data). EmitCallbackTrampoline
    // checks 'if (!cookie) return' — passing nullptr would silently drop all callbacks.
    QaiorSS_Status_t status = fn_enqueue(pipeline, context, &frame, this);
    if (status != QAIOR_SS_STATUS_OK) {
...
int SmartSelectionAdapter::PrepareFrameParcelFd(int parcel_fd,
...
int SmartSelectionAdapter::PrepareFrameGraphicBuffer(void* buffer,
                                                      const std::string& metadata_json,
                                                      QaiorSS_FrameHandle_t& frame) {
    frame = {};
...
int SmartSelectionAdapter::PrepareFrameNativeHandle(
    ...
    if (snapmapper) {
        ...
        auto ret_fmt  = snapmapper->GetMetadata(*snap_hdl,
                            MetadataType::PIXEL_FORMAT_ALLOCATED, &queried_format);
        auto ret_ubwc = snapmapper->GetMetadata(*snap_hdl,
                            MetadataType::IS_UBWC, &queried_ubwc);
        ...
        if (gralloc_w != 0 && gralloc_h != 0) {
            DLOGI("PrepareFrameNativeHandle: gralloc dims=%ux%u stride=%u",
                  gralloc_w, gralloc_h, gralloc_stride);
        } else {
    ...
    // -----------------------------------------------------------------------
    uint32_t w      = (gralloc_w != 0) ? gralloc_w
                                        : ExtractUint32FromJson(metadata_json, "width");
    uint32_t h      = (gralloc_h != 0) ? gralloc_h
                                        : ExtractUint32FromJson(metadata_json, "height");
    uint32_t stride = (gralloc_stride != 0) ? gralloc_stride
                                             : ExtractUint32FromJson(metadata_json, "stride");

    if (w == 0 || h == 0) {
        DLOGE("PrepareFrameNativeHandle: cannot determine buffer dimensions "
              "(gralloc failed and metadata_json missing width/height)");
        return -EINVAL;
    }
    DLOGI("PrepareFrameNativeHandle: using dims=%ux%u stride=%u (ubwc=%lld)",
          w, h, stride, static_cast<long long>(is_ubwc));
    ...
    if (gralloc_format_valid) {
        ...
        DLOGI("PrepareFrameNativeHandle: AHB format from gralloc 0x%x → 0x%x",
              gralloc_format, ahb_format);
    } else {
    ...
    int ahb_ret = AHardwareBuffer_createFromHandle(&desc, native_hdl,
                      AHARDWAREBUFFER_CREATE_FROM_HANDLE_METHOD_CLONE, &ahb);
    if (ahb_ret != 0 || !ahb) {
        DLOGE("PrepareFrameNativeHandle: AHardwareBuffer_createFromHandle(CLONE) failed: %d",
              ahb_ret);
        return -EINVAL;
    }
    ...
    // -----------------------------------------------------------------------
    {
        ...
        DLOGI("PrepareFrameNativeHandle: AHB actual desc w=%u h=%u fmt=0x%x stride=%u",
              actual.width, actual.height, actual.format, actual.stride);
        bool mismatch = (actual.width != desc.width || actual.height != desc.height ||
                         actual.format != desc.format ||
                         (actual.stride != 0 && actual.stride != desc.stride));
        if (mismatch) {
            DLOGW("PrepareFrameNativeHandle: AHB descriptor mismatch! "
    ...
    // -----------------------------------------------------------------------
    {
        ...
        int lock_ret = AHardwareBuffer_lock(ahb, AHARDWAREBUFFER_USAGE_CPU_READ_OFTEN,
                                            -1, nullptr, &cpu_data);
        if (lock_ret != 0 || !cpu_data) {
            DLOGW("PrepareFrameNativeHandle: AHardwareBuffer_lock() failed (ret=%d) — "
            ...
            for (int i = 0; i < 16 && all_zero; i++) {
                if (px[i] != 0) all_zero = false;
            }
            DLOGI("PrepareFrameNativeHandle: AHB lock OK cpu_data=%p "
...
int SmartSelectionAdapter::DoDeleteByConfig(const sdm::GenericPayload& input,
    ...
    QaiorSS_Status_t status = fn_delete(pipeline, context, json_copy.c_str(),
    ...
    if (out_frames) {
        ...
        bool has_output = (output &&
                           output->GetPayload(delete_result, &payload_size) == 0 &&
        ...
        for (int32_t i = 0; i < out_count; ++i) {
            ...
            if (has_output) {
                ...
                if (f.type == QAIOR_SS_FRAME_PARCELFD && f.parcelFd >= 0) {
                    ...
                    if (found) AHardwareBuffer_release(ahb);
                }
            }
        }
        fn_free(out_frames);
...
int SmartSelectionAdapter::DoDeinit(const sdm::GenericPayload& input,
                                    sdm::GenericPayload* output) {
    DLOGD("DoDeinit");
...

    {
        ...
        pipeline  = pipeline_;  context  = context_;
        pipeline_ = nullptr;    context_ = nullptr;
...
QaiorSS_FrameHandle_t SmartSelectionAdapter::BuildFrameHandleFromSnapHandle(
    ...
    if (stride == 0 || width == 0 || height == 0) {
        DLOGW("BuildFrameHandleFromSnapHandle: critical metadata may be invalid "
    ...
    DLOGV("BuildFrameHandleFromSnapHandle: fd=%d size=%llux%llu stride=%u app=%s",
          frame.parcelFd,
          static_cast<unsigned long long>(width),
          static_cast<unsigned long long>(height),
          stride, app_name.c_str());
...
int SmartSelectionAdapter::LoadConfigurationJson(const std::string& config_path) {
    if (config_path.empty()) return -EINVAL;
    ...
    DLOGD("Loaded configuration from %s (%zu bytes)",
          config_path.c_str(), current_config_json_.size());
    return 0;
}
