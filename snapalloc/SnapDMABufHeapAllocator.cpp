...
SnapDMABufHeapAllocator *SnapDMABufHeapAllocator::instance_{nullptr};
std::mutex SnapDMABufHeapAllocator::snap_dma_alloc_mutex_;

SnapDMABufHeapAllocator::SnapDMABufHeapAllocator() {
  debug_ = Debug::GetInstance();
}

SnapDMABufHeapAllocator *SnapDMABufHeapAllocator::GetInstance() {
  std::lock_guard<std::mutex> lock(snap_dma_alloc_mutex_);

  if (instance_ == nullptr) {
    instance_ = new SnapDMABufHeapAllocator();
  }
  return instance_;
}

Error SnapDMABufHeapAllocator::AllocBuffer(AllocData *ad) {
...
Error SnapDMABufHeapAllocator::FreeBuffer(void *base, unsigned int size, int fd,
...
Error SnapDMABufHeapAllocator::UnmapBuffer(void *base, unsigned int size) {
  auto err = Error::NONE;
  if (munmap(base, size)) {
    err = Error::BAD_VALUE;
...
Error SnapDMABufHeapAllocator::MapBuffer(void **base, unsigned int size, int fd) {
  auto err = Error::NONE;
...
Error SnapDMABufHeapAllocator::CleanBuffer(void * /*base*/, unsigned int /*size*/, int op,
                                    int dma_buf_fd) {
...
int SnapDMABufHeapAllocator::ImportBuffer(int fd) {
  return fd;
}

Error SnapDMABufHeapAllocator::SecureMemPerms(AllocData *ad) {
  int ret = 0;
...
void SnapDMABufHeapAllocator::GetHeapInfo(vendor_qti_hardware_display_common_BufferUsage usage,
...
void SnapDMABufHeapAllocator::GetVMPermission(vendor_qti_hardware_display_common_BufferPermission buf_perm,
...
Error SnapDMABufHeapAllocator::SetBufferPermission(
...
void SnapDMABufHeapAllocator::Deinit() {
...
void SnapDMABufHeapAllocator::GetCSFVersion() {
  if (csf_initialized_) {
    return;
  }
...
bool SnapDMABufHeapAllocator::CSFEnabled() {
#ifdef TARGET_USES_SMMU_PROXY
