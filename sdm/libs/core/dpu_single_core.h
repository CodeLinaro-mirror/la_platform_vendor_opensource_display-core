...
#ifndef __DPU_SINGLE_CORE_H__
#define __DPU_SINGLE_CORE_H__

#include "dpu_core_mux.h"

namespace sdm {

class DPUSingleCore : public DPUCoreMux {
 public:
  DisplayError Destroy();

  DPUSingleCore(DisplayId display_id, SDMDisplayType type,
 ...
 private:
  ...
  inline void Resize(DisplayDeviceContext *device_ctx) {
    if (device_ctx->size() != 1) {
      device_ctx->insert({core_id_, {}});
    }
  }
};
